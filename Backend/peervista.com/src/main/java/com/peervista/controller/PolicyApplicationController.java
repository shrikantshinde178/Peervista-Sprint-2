...
//    
//    @GetMapping("fetch-by/userId/{user_id}")
//    public ResponseEntity<List<PolicyApplication>> UserBuyedPolicies(PolicyApplication user_id){
//    	List<PolicyApplication> buyedPolicies = policyApplicationService.UserBuyedPolicies(user_id);
//    	return ResponseEntity.ok(buyedPolicies);
//    }
   
    @GetMapping("/fetch-policy/userId/{userId}")
    public ResponseEntity<List<PolicyApplication>> UserBuyedPolicies(@PathVariable("userId") Long userId) {
        List<PolicyApplication> buyedPolicies = policyApplicationService.UserBuyedPolicies(userId);
        return ResponseEntity.ok(buyedPolicies);
    }
    
    @GetMapping("/users-by-policy/{policyId}")
    public List<User> getUsersByPolicyId(@PathVariable Long policyId) {
        return policyApplicationService.getUsersByPolicyId(policyId);
    }
